...
target_bop_objs, obj_forced = bproc.loader.load_bop_objs(bop_dataset_path = os.path.join(args.bop_parent_path, 'ycbv'), object_model_unit='mm', id_to_include=None)

# load distractor bop objects
tless_dist_bop_objs, _ = bproc.loader.load_bop_objs(bop_dataset_path = os.path.join(args.bop_parent_path, 'tless'), model_type = 'cad', mm2m = True)
hb_dist_bop_objs, _ = bproc.loader.load_bop_objs(bop_dataset_path = os.path.join(args.bop_parent_path, 'hb'), mm2m = True)
tyol_dist_bop_objs, _ = bproc.loader.load_bop_objs(bop_dataset_path = os.path.join(args.bop_parent_path, 'tyol'), mm2m = True)
...
for i in range(args.num_scenes):
    ...
    sampled_target_bop_objs = list(np.random.choice(target_bop_objs, size=6, replace=False))
    ...
    sampled_distractor_bop_objs += list(np.random.choice(tyol_dist_bop_objs, size=1, replace=False))

    if obj_forced != None and obj_forced not in sampled_target_bop_objs:
        print(f'Including the object {obj_forced.get_cp("category_id")} to balance the dataset')
        sampled_target_bop_objs[-1] = obj_forced

    for obj in sampled_target_bop_objs:
        print(f'Object {obj.get_cp("category_id")} will be part of the scene')
    ...
    for obj in (sampled_target_bop_objs + sampled_distractor_bop_objs):        
        ...
        if obj.get_cp("bop_dataset_name") == 'ycbv':
    ...
    while cam_poses < 25:
        ...
        poi = bproc.object.compute_poi(np.random.choice(sampled_target_bop_objs, size=len(sampled_target_bop_objs), replace=False))
    ...
    print(f'\n\n COMPLETED {i}/{args.num_scenes} SCENES')
