...
def load_bop_objs(bop_dataset_path: str, model_type: str = "", obj_ids: Optional[List[int]] = None,
                  ...
                  move_origin_to_x_y_plane: bool = False, id_to_include: int = None) -> List[MeshObject]:
    ...
    if bop_dataset_name == 'ycbv' and not os.path.exists(os.path.join(bop_dataset_path, '..', 'models_info.json')):
        calc_model_info(os.path.join(bop_dataset_path, 'models'))
    ...
    if bop_dataset_name == 'ycbv':
        ply_files = [file for file in os.listdir(os.path.join(bop_dataset_path, 'models')) if file.endswith('.ply')]
        model_p['obj_ids'] = list(range(1,len(ply_files)))
    ...
    if sample_objects:
        ...
        for obj_id in obj_ids:
            ...
            loaded_objects.append(cur_obj)
            if id_to_include != None and obj_id == id_to_include:
                obj_forced = cur_obj
    ...
    return loaded_objects, obj_forced
...
def calc_model_info(models_path):
    ...
    for obj_id in dp_model["obj_ids"]:
        ...
        ref_pt = model["pts"].min(axis=0)
        size = model["pts"].max(axis=0) - ref_pt

        # Calculated diameter.
        diameter = misc.calc_pts_diameter(model["pts"])

        models_info["obj_{obj_id:06d}".format(obj_id=obj_id)] = {
            "min_x": ref_pt[0],
            "min_y": ref_pt[1],
            "min_z": ref_pt[2],
            "size_x": size[0],
