...
def test(data,
    ...
    if nc > 1 and len(stats):
        for i, c in enumerate(ap_class):
            ...
            log_file = Path(log_dir) / 'class_results.txt'
            # Open the file in append mode
            with open(log_file, 'w') as f:
                f.write(f"{'Class Name':<25}{'Num. Images':<20}{'Class Instances':<20}{'Precision':<20}{'Recall':<20}{'mAP@0.5':<20}{'mAP@0.5:0.95':<20}\n")
                
                # Write the values
                for i, c in enumerate(names):
                    f.write(f"{c:<25}{seen:<20}{nt[i]:<20}{p[i]:<20.4f}{r[i]:<20.4f}{ap50[i]:<20.4f}{ap[i]:<20.4f}\n")
