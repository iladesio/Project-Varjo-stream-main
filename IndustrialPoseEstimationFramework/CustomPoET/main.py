...
def main(args):
    ...
    #dataset_val = build_dataset(image_set=args.train_set, args=args)
    ...
    if args.resume:
        ...
        # Remove the old class-specific layers weights from the checkpoint (if they exist)
        #checkpoint['model'] = {k: v for k, v in checkpoint['model'].items() if 'translation_head' not in k and 'rotation_head' not in k}

        missing_keys, unexpected_keys = model_without_ddp.load_state_dict(checkpoint['model'], strict=False)
        unexpected_keys = [k for k in unexpected_keys if not (k.endswith('total_params') or k.endswith('total_ops'))]

        # Properly initialize the new layers
        # for layer in [model.translation_head, model.rotation_head]:
