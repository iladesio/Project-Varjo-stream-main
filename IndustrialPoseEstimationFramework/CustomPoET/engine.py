...
def pose_evaluate(model, matcher, pose_evaluator, data_loader, image_set, bbox_mode, rotation_mode, device, output_dir, epoch=None):
    ...
    add_results = pose_evaluator.evaluate_pose_add(output_eval_dir)
    print("Start Calculating ADD-S")
    adi_results = pose_evaluator.evaluate_pose_adi(output_eval_dir)
    print("Start Calculating ADD(-S)")
    adds_results = pose_evaluator.evaluate_pose_adds(output_eval_dir)
    print("Start Calculating Average Translation Error")
    cls_translation_errors, avg_translation_errors = pose_evaluator.calculate_class_avg_translation_error(output_eval_dir)
    print("Start Calculating Average Rotation Error")
    cls_rotation_errors, avg_rotation_errors= pose_evaluator.calculate_class_avg_rotation_error(output_eval_dir)
    total_time = time.time() - start_time
    total_time_str = str(datetime.timedelta(seconds=int(total_time)))
